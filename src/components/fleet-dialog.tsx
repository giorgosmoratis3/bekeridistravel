      ...
      <DialogContent className="max-w-sm sm:max-w-md p-0 overflow-hidden border border-ink/10 bg-white shadow-2xl rounded-2xl [&>button]:hidden">
        {fleet && (
          <div className="relative max-h-[85vh] flex flex-col">
            <button type="button" onClick={onClose} aria-label={t("Κλείσιμο", "Close")}
            ...
            <div className="p-5 sm:p-6 overflow-y-auto">
              <DialogHeader className="sr-only">
                <DialogTitle>
                  {fleet.type} {fleet.seats} {t("θέσεων", "seats")}
                </DialogTitle>
              ...
              <div className="relative h-36 sm:h-44 overflow-hidden rounded-xl border border-ink/10 bg-ink/5">
                <img src={fleet.img} alt={`${fleet.type} ${fleet.seats} ${t("θέσεων", "seats")}`}
              ...
              <p className="mt-3 text-ink/75 leading-relaxed text-[13px]">{fleet.details}</p>

              <div className="mt-4 grid grid-cols-2 gap-1.5">
                {fleet.specs.map(({ icon: Icon, label, value }) => (
                  <div key={label} className="rounded-lg border border-ink/10 bg-ink/[0.03] p-2.5">
                    <div className="flex items-center gap-1.5 text-ink/55 text-[8px] tracking-[0.2em] font-display uppercase">
                      <Icon size={10} strokeWidth={1.8} />
                      {label}
                    </div>
                    <div className="mt-0.5 text-ink text-[12px] font-medium">{value}</div>
                  </div>
                ))}
              </div>
              ...
              <div className="mt-4">
                <div className="font-display text-[9px] tracking-[0.3em] text-brand mb-2">
                  {t("ΕΞΟΠΛΙΣΜΟΣ", "EQUIPMENT")}
                </div>
                <div className="flex flex-wrap gap-1.5">
                  {fleet.features.map(({ icon: Icon, label }) => (
                    <span key={label} className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border border-ink/15 bg-white text-[10px] text-ink/80">
